import { useCuentasBancariasPlanta } from "../../hooks/useCuentasBancariasPlanta";
...
import { RegistroCuenta } from "./components/registro-cuenta.tsx";
import { CuentaBancaria } from "./components/cuenta-bancaria.tsx";
...
export const CuentasBancarias = ({ planta, onCuentasCountChange }: Props) => {
  const {
    cuentas,
    bancos,
    setBancos,
    ...
    insertCuenta,
    toggleEstadoCuenta,
  } = useCuentasBancariasPlanta(planta.id, onCuentasCountChange);

  const [searchQuery, setSearchQuery] = useState("");
  const [openAgregar, setOpenAgregar] = useState(false);
  const [cuentaAEditar, setCuentaAEditar] = useState<CuentaBancariaPlantaResponse | null>(null);
    ...
    <div className="flex flex-col gap-6">
      ...
      <Group justify="space-between" align="center" gap="md" wrap="nowrap">
        ...
        <Button
          ...
          className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-955/30 shrink-0"
        >
          Agregar Cuenta
        </Button>
      </Group>
      ...
      >
        <RegistroCuenta
          ...
          onCuentaAdded={(newAccount: CuentaBancariaPlantaResponse) => {
            insertCuenta(newAccount);
            setOpenAgregar(false);
          }}
          onBancoAdded={(newBanco: RES_Banco) => {
      ...
      >
        {cuentaAEditar && (
          <RegistroCuenta
            ...
            onCuentaAdded={(updatedAccount: CuentaBancariaPlantaResponse) => {
