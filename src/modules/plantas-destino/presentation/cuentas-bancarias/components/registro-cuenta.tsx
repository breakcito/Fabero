...
interface Props {
  ...
  onCuentaAdded: (account: CuentaBancariaPlantaResponse) => void;
...
export const RegistroCuenta = forwardRef<RegistroCuentaRef, Props>(
  (
    { idPlanta, bancos, loadingBancos, onCuentaAdded, onBancoAdded, cuenta },
    ref,
  ) => {
    const {
      payload,
      ...
      error,
      autoSelectBanco,
    } = useRegistroCuentaBancaria(idPlanta, bancos, onCuentaAdded, cuenta);
      ...
      <div className="bg-zinc-900/50 p-5 rounded-xl border border-zinc-800 relative overflow-hidden">
        ...
        >
          <RegistroBanco
            ...
            onSuccess={(banco) => {
              onBancoAdded(banco);
              autoSelectBanco(banco.id_banco);
              setOpenBanco(false);
            }}
          />
        </ModalEstandar>
