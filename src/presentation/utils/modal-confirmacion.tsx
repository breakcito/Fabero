...
interface ConfirmacionBodyProps {
  message: React.ReactNode;
  confirmLabel: string;
  cancelLabel: string;
  tipo: "info" | "peligro";
...
export const mostrarConfirmacion = ({
  ...
  modals.open({
    ...
    classNames: {
      content:
        "bg-zinc-950/95 border border-zinc-800/80 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.8)] backdrop-blur-md rounded-[24px] overflow-hidden",
      header:
        "bg-zinc-950 text-white pt-5 pb-3 px-6 border-b border-zinc-900/50 flex items-center justify-between",
      body: "bg-zinc-950 px-6 pt-5 pb-6",
      close:
        "text-zinc-400 hover:text-white hover:bg-white/10 transition-all duration-200 rounded-full w-8 h-8 flex items-center justify-center border border-transparent hover:border-zinc-800/50",
    },
    children: (
      <ConfirmacionBody
        message={message}
